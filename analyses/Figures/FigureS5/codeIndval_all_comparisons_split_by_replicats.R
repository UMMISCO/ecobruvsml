# =============================================================================
# Script name: codeIndval_all_comparisons_split_by_replicats.R
# Author: Estephe Kana, Eugeni Belda, Edi Prifti
# Date created: 2024-12-01
# Purpose: Run IndVal analysis on BRUVS data split by replicates across all
...
# Outputs: Figures/FigureS5/results_replicas_analysis/Indval_output_data/
...
# Rscript codeIndval_all_comparisons_split_by_replicats.R 10 MBERE ABORE
# -----------------------------------------------------------------------------
...
permanova_analysis_indval <- function(sample.info, sample.X, sample.class, results_indval, hab_type, ilevels, data_type) {
  ...
  if (data_type == "maxN") {
    ...
    subdf.features.df <- sample.X[, indics.species, drop=FALSE]
...
save_path_indval <- file.path(base_dir, "analyses", "Figures", "FigureS5", "results_replicas_analysis", "Indval_output_data", "")
...
file_path_indval <- paste0(save_path_indval, "Indval_all_comparison_replicas_", paste(replica1, "vs", replica2, sep="_"), "_prev_", prevalence_rate, ".Rda")
...
if (!file.exists(file_path_indval)) {
  ...
  {
    # save indval results
    indvalout.bin[[paste(ilevels_hab, collapse = "_")]] <- save_indval_results(ilevels.df.indval, Comp_data, ilevels_hab)
    ##Analyse permanova; get the species to include (indval pvalue<0.05)
