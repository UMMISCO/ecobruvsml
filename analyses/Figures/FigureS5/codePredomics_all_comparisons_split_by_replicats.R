# =============================================================================
# Script name: codePredomics_all_comparisons_split_by_replicats.R
# Author: Estephe Kana, Eugeni Belda, Edi Prifti
# Date created: 2024-12-01
# Purpose: Run Predomics (bininter/terinter) analysis on BRUVS data split by
#          replicates across all habitat comparisons. Takes command-line
#          arguments for algorithm language, prevalence threshold, and replica IDs.
# Inputs:  data/AllDataexportefixed.txt
# Outputs: Figures/FigureS5/results_replicas_analysis/<algo>_output_data/
...
# Rscript codePredomics_all_comparisons_split_by_replicats.R terinter 10 MBERE ABORE
# -----------------------------------------------------------------------------
...
replica1 <- args[3] # train models on replica1
replica2 <- args[4] # test models on replica2
...
permanova_analysis_predomics <- function(sample.info, sample.X, sample.class, results_pred, hab_type, ilevels, algorithm_language, data_type) {
  ...
  if (data_type == "maxN") {
    ...
    subdf.features.df <- sample.X[, fbm.species, drop=FALSE]
    
    # Exclude samples with no species
    subdf.features.df <- subdf.features.df[rowSums(subdf.features.df) > 0, , drop=FALSE]
...
for(i in 1:length(comp))
{
  ...
  # Train data on replica1
  ilevels.df.train <- ilevels.df[ilevels.df$Transect==replica1,]
  ...
  ## test data on replica2
  ilevels.df.test <- ilevels.df[ilevels.df$Transect==replica2,]
  # prepare data for analysis
...
save_path_pred <- file.path(base_dir, "analyses", "Figures", "FigureS5", "results_replicas_analysis", paste0(algorithm_language, "_output_data"), "")
...
file_path <- paste0(save_path_pred, algorithm_language, "_Predomics_all_comparison_replicas_", paste(replica1, "vs", replica2, sep="_"), "_prev_", prevalence_rate, ".Rda")
