...
# Purpose: Generate Figure 4 - PERMANOVA results and model performance plots
#          (radar/dot plots) comparing IndVal, Twinspan, bininter, terinter
#          methods across habitat comparisons.
# Inputs:  analysis_outputs/Indval_output_data/Indval_all_analyses_overall_data_prev_10.Rda
#          analysis_outputs/bininter_output_data/bininter_Predomics_all_analyses_overall_data_prev_10.Rda
#          analysis_outputs/terinter_output_data/terinter_Predomics_all_analyses_overall_data_prev_10.Rda
#          analysis_outputs/twinspan_output_data/Twinspan_all_analyses_overall_data_prev_10.Rda
#          analysis_outputs/video_data_object.rda
#          analysis_outputs/auc_acc_species_prev_10.csv
# Outputs: Figures/Figure4/Figure4.pdf
# =============================================================================

# -----------------------------------------------------------------------------
# Commands to run the script:
# cd /home/estephe/Documents/Papers/2026-aime_key_fish_bruvs/Bruvs-analysis-code/analyses/Figures/Figure4
# Rscript Figure4_code.R
# -----------------------------------------------------------------------------
...
# load indval results
load(file=file.path(base_dir, "analyses", "analysis_outputs", "Indval_output_data", "Indval_all_analyses_overall_data_prev_10.Rda"))
adonis_indval.bin <- adonis.bin; adonis_indval.maxn <- adonis.maxn

# load bininter results
load(file=file.path(base_dir, "analyses", "analysis_outputs", "bininter_output_data", "bininter_Predomics_all_analyses_overall_data_prev_10.Rda"))
...
load(file=file.path(base_dir, "analyses", "analysis_outputs", "terinter_output_data", "terinter_Predomics_all_analyses_overall_data_prev_10.Rda"))
# Rename variables loaded to diffenriate it with terinter vars
...
# load twinspan results
load(file=file.path(base_dir, "analyses", "analysis_outputs", "twinspan_output_data", "Twinspan_all_analyses_overall_data_prev_10.Rda"))
adonis_twinspan.bin <- adonis.bin; adonis_twinspan.maxn <- adonis.maxn
twinspanout.bin.sub <- twinspan.bin.sub; twinspanout.maxn.sub <- twinspan.maxn.sub
rm(adonis.bin, adonis.maxn, twinspan.bin, twinspan.maxn, twinspan.bin.sub, twinspan.maxn.sub)

# initialize a list of adonis objects  
adonis.list <- list(
  ind.bin= adonis_indval.bin, ind.maxn= adonis_indval.maxn,
  pred_bin.bin= adonis_pred_bininter.bin, pred_bin.maxn= adonis_pred_bininter.maxn,
  pred_ter.bin= adonis_pred_terinter.bin, pred_ter.maxn= adonis_pred_terinter.maxn,
  twinspan.bin = adonis_twinspan.bin, twinspan.maxn = adonis_twinspan.maxn
)
...
indics_species.list <- list(indval.bin =indvalout.bin.sub, indval.maxn= indvalout.maxn.sub,
                            ...
                            twinspan.bin= twinspanout.bin.sub, twinspan.maxn= twinspanout.maxn.sub)
...
#pdf(file=file.path(base_dir, "analyses", "Figures", "Figure4", "permanova_all_results_updated_final.pdf"), h=6, w=7)
...
#pdf(file=file.path(base_dir, "analyses", "Figures", "Figure4", "permanova_ratio_all_results_final.pdf"), h=6, w=7)
...
ggplot(data=adonis.df_IN_OFF, aes(x=data, y=ratio, fill=source, color=colour)) +
  ...
  labs(color = "Significativity") +
  scale_color_manual(values = c('blue', 'red'), labels =  c( 'pval <= 0.001', 'pval > 0.001 & pval <=0.05')) +
  scale_y_continuous(name="r2_comp/r2_community")

# plot number of indics species by method on inshore-offshore
# ggplot(data=adonis.df_filt_IN_OFF, aes(x=source, y=features, fill=source)) +
#  geom_bar(stat="identity", position = "dodge")+
...
#  scale_x_discrete(labels=c("Pred_Bininter", "Indval", "Pred_Terinter"))

adonis.df_filt_IN_OFF$source <- factor(adonis.df_filt_IN_OFF$source, 
                                       levels = c("indvalSpecies", "terinterSpecies", "bininterSpecies"))
...
indics_species.list[indics_species.list == 'terinter'] <- 'Pred_Terinter'
indics_species.list[indics_species.list == 'bininter'] <- 'Pred_Bininter'
indics_species.list[indics_species.list == 'terinter'] <- 'Pred_Terinter'
indics_species.list[indics_species.list == 'bininter'] <- 'Pred_Bininter'
...
common.sp.maxn <- intersect(intersect(venn_list_by_method[['maxN']]$indval, venn_list_by_method[['maxN']]$pred_bin), venn_list_by_method[['maxN']]$pred_ter)

# get common species names for pres/abs table
common.sp.bin <- intersect(intersect(venn_list_by_method[['pres/abs']]$indval, venn_list_by_method[['pres/abs']]$pred_bin), venn_list_by_method[['pres/abs']]$pred_ter)
...
# pdf(file=file.path(base_dir, "analyses", "Figures", "Figure4", "models_performances_pred_video_data_pa.pdf"), h=4, w=6)
...
auc_acc_species_prev_10.list <- list()
auc_acc_species_prev_10.list$generalization.auc <- digest.list$generalization.auc
auc_acc_species_prev_10.list$empirical.auc <- digest.list$auc_
auc_acc_species_prev_10.list$generalization.acc <- digest.list$generalization.acc
auc_acc_species_prev_10.list$empirical.acc <- digest.list$accuracy_

# match dataframe structure
for (i in names(auc_acc_species_prev_10.list)) {
  if( i %in% c("generalization.auc", "generalization.acc")){
    auc_acc_species_prev_10.list[[i]] = auc_acc_species_prev_10.list[[i]][, !names(auc_acc_species_prev_10.list[[i]]) %in% "variable"]
  }
  auc_acc_species_prev_10.list[[i]][["metric"]] <- i
  # reorder the columns
  auc_acc_species_prev_10.list[[i]] <- auc_acc_species_prev_10.list[[i]][, c("metric", "value","parsimony","comparison","source","algorithm")]
}
...
# rbind auc and acc perf data of auc_acc_species_prev_10.list

auc_acc_species_prev_10.df <- flatten_and_bind(auc_acc_species_prev_10.list)
rownames(auc_acc_species_prev_10.df) <- NULL

# add a column for dataset
auc_acc_species_prev_10.df$dataset <- "species_prev_10"

# save model performances
write.csv(auc_acc_species_prev_10.df, file=file.path(base_dir, "analyses", "analysis_outputs", "auc_acc_species_prev_10.csv"))
...
# save the results for inshore-offshore comparision
pdf(file=file.path(base_dir, "analyses", "Figures", "Figure4", "Figure4.pdf"), h=12, w=15)
wrap_plots(wrap_elements(fbm.plots$INSHORE_OFFSHORE$maxN), wrap_elements(fbm.plots$INSHORE_OFFSHORE$presAbs), nrow = 2) + plot_annotation(tag_levels = "A")
dev.off()
...
summary_stats <- all_species_IN_OFF.df %>%
  group_by(source, data, IsIndSp) %>%
  summarise(
    mean_prevalence = mean(prevalence, na.rm = TRUE), # calculate the maen
    sd_prevalence = sd(prevalence, na.rm = TRUE), # calculate the Standard Deviation
    total_species = n(),  # Count the number of species
    se_prevalence = sd_prevalence / sqrt(total_species)  # Calculate Standard Error
  )
# rename legend values
summary_stats$IsIndSp <- factor(summary_stats$IsIndSp, levels=c(0,1), labels = c("No","Yes"))

# plot the prevalence distribution with mean and standard error
prev_distribution_update <- ggplot(summary_stats, aes(x=source, y=mean_prevalence, color=IsIndSp)) +
  geom_point(size=3, position=position_dodge(width=0.6)) +
  geom_errorbar(aes(ymin=mean_prevalence - se_prevalence, ymax=mean_prevalence + se_prevalence), width=0.3, position=position_dodge(width=0.6)) +
  scale_color_manual(values = c("black", "grey"))+
  labs(x = "Methods", y = "Mean prevalence", color= "Indicator species") +
  facet_grid(. ~data) +
  theme_bw() +
  theme(legend.position = "bottom",
...
# boxplot of prevalence distribution 
# rename lagend values
# all_species_IN_OFF.df$IsIndSp <- factor(all_species_IN_OFF.df$IsIndSp, levels=c(0,1), labels = c("No","Yes"))
# 
# prev_distribution <- ggplot(all_species_IN_OFF.df, aes(x=source, y=prevalence, color=IsIndSp)) +
#   geom_boxplot(alpha=0.6) +
#   labs(x = "Methods", y = "Prevalence", color= "Indicator species") +
#   facet_grid(. ~data) +
#   theme_bw() +
#   stat_compare_means(method = "wilcox.test", label = "p.signif", hide.ns = TRUE)
...
## Correlation between indicator value and feature importance
all_species_IN_OFF.df$IsIndSp= as.factor(all_species_IN_OFF.df$IsIndSp)
...
merged_indval_ter = merge(all_species_IN_OFF_indval.df, all_species_IN_OFF_terinter.df, by=c("feature","comparison","data"), all.x = TRUE)
...
merged_indval_bin$color= ifelse(merged_indval_bin$IsIndSp.x==1 & merged_indval_bin$IsIndSp.y==0,
                                -1, 
                                ifelse(merged_indval_bin$IsIndSp.x==1 & merged_indval_bin$IsIndSp.y==1, 0, 1))
merged_indval_bin$color <- factor(merged_indval_bin$color, levels=c(-1,0,1), labels = c("indVal-only","indVal-Pred_Bininter","Pred_Bininter-only"))

# get indics_species from indval and terinter
#indval_terinter_species= rbind(indval_species, terinter_species)
merged_indval_ter$color= ifelse(merged_indval_ter$IsIndSp.x==1 & merged_indval_ter$IsIndSp.y==0,
                                -1, 
                                ifelse(merged_indval_ter$IsIndSp.x==1 & merged_indval_ter$IsIndSp.y==1, 0, 1))
merged_indval_ter$color <- factor(merged_indval_ter$color, levels=c(-1,0,1), labels = c("indVal-only","indVal-Pred_Terinter","Pred_Terinter-only"))
...
# Ensure data is filtered for ggpairs compatibility
indval_bininter_species.sub <- unique(merged_indval_bin) %>%
  rename(featImportance.Pred_Bininter = featureImportance) %>%
  select(featImportance.Pred_Bininter, indicator_value, color, data)  # Select relevant variables

# Convert categorical variables (if necessary)
indval_bininter_species.sub$color <- as.factor(indval_bininter_species.sub$color)
indval_bininter_species.sub$data <- as.factor(indval_bininter_species.sub$data)

# Create ggpairs plot
indval.feat1 <- ggpairs(indval_bininter_species.sub, columns = c("featImportance.Pred_Bininter", "indicator_value", "data"),
                        aes(color = color, alpha = 0.5)) +  # Adjust threshold if needed
  labs(title = "Indval vs Predomics_Bininter")+
  scale_colour_manual(values = c("#648C67","black","#CC3837")) +
  scale_fill_manual(values = c("#648C67","black","#CC3837")) +
  theme_bw()
...
# Ensure data is filtered for ggpairs compatibility
indval_terinter_species.sub <- unique(merged_indval_ter) %>%
  rename(featImportance.Pred_Terinter = featureImportance) %>%
  select(featImportance.Pred_Terinter, indicator_value, color, data)  # Select relevant variables

# Convert categorical variables (if necessary)
indval_terinter_species.sub$color <- as.factor(indval_terinter_species.sub$color)
indval_terinter_species.sub$data <- as.factor(indval_terinter_species.sub$data)

# Create ggpairs plot
indval.feat2 <- ggpairs(indval_terinter_species.sub, columns = c("featImportance.Pred_Terinter", "indicator_value", "data"),
                        aes(color = color, alpha = 0.5)) + 
  labs(title = "Indval vs Predomics_Terinter")+
  scale_colour_manual(values = c("#648C67","black","#8195B2")) +
  scale_fill_manual(values = c("#648C67","black","#8195B2")) +
  theme_bw()

indval.feat2

# merge Indval, featImportance.Pred_bininter and featImportance.Pred_terinter in the same ggpairs
#merged_indval_bin_ter= merge(all_species_IN_OFF_indval.df, all_species_IN_OFF_bininter.df, all_species_IN_OFF_terinter.df, by= c("feature","comparison","data"), all = TRUE)

merged_indval_bin_ter <- Reduce(function(x, y) merge(x, y, by = c("feature","comparison","data", "prevalence"), all = TRUE), list(all_species_IN_OFF_indval.df, all_species_IN_OFF_bininter.df, all_species_IN_OFF_terinter.df))
print(merged_indval_bin_ter)

merged_indval_bin_ter$IndSp_by_method <- ifelse(merged_indval_bin_ter$IsIndSp.x == 1 & merged_indval_bin_ter$IsIndSp == 1 & merged_indval_bin_ter$IsIndSp.y == 1 , "all_methods",
                                                ifelse(merged_indval_bin_ter$IsIndSp.x == 0 & merged_indval_bin_ter$IsIndSp == 1 & merged_indval_bin_ter$IsIndSp.y == 1, "Pred_Terinter-Pred_Bininter",
                                                       ifelse(merged_indval_bin_ter$IsIndSp.x == 1 & merged_indval_bin_ter$IsIndSp == 0 & merged_indval_bin_ter$IsIndSp.y == 1, "Indval-Pred_Bininter",
                                                              ifelse(merged_indval_bin_ter$IsIndSp.x == 1 & merged_indval_bin_ter$IsIndSp == 1 & merged_indval_bin_ter$IsIndSp.y == 0, "Indval-Pred_Terinter",
                                                                     ifelse(merged_indval_bin_ter$IsIndSp.x == 1 & merged_indval_bin_ter$IsIndSp == 0 & merged_indval_bin_ter$IsIndSp.y == 0, "Indval-only",
                                                                            ifelse(merged_indval_bin_ter$IsIndSp.x == 0 & merged_indval_bin_ter$IsIndSp == 0 & merged_indval_bin_ter$IsIndSp.y == 1, "Pred_Bininter-only",
                                                                                   ifelse(merged_indval_bin_ter$IsIndSp.x == 0 & merged_indval_bin_ter$IsIndSp == 1 & merged_indval_bin_ter$IsIndSp.y == 0, "Pred_Terinter-only", NA)
                                                                            )
                                                                     )
                                                              )
                                                       )
...
library(dplyr)
library(GGally)
indval_bin_ter_species.sub <- unique(merged_indval_bin_ter) %>%
  rename(featImportance.Pred_Terinter = featureImportance.y, featImportance.Pred_Bininter = featureImportance.x) %>%
  drop_na(IndSp_by_method) %>%  # Remove rows where IndSp_by_method is NA
  select(featImportance.Pred_Bininter, featImportance.Pred_Terinter, indicator_value, IndSp_by_method, data)  # Select relevant variables

# Convert categorical variables (if necessary)
indval_bin_ter_species.sub$IndSp_by_method <- as.factor(indval_bin_ter_species.sub$IndSp_by_method)
indval_bin_ter_species.sub$data <- as.factor(indval_bin_ter_species.sub$data)

# Create ggpairs plot
indicator_value.featImport.plot <- ggpairs(indval_bin_ter_species.sub, columns = c("featImportance.Pred_Bininter", "featImportance.Pred_Terinter", "indicator_value", "data"),
                                           aes(color = IndSp_by_method, alpha = 0.5)) + 
  labs(title = "Indicator_value vs Feature_Importance")+
  scale_colour_manual(values = c("#9e4e44","#413a7b","#648C67","black","purple","#8195B2")) +
  scale_fill_manual(values = c("#9e4e44","#413a7b","#648C67","black","purple","#8195B2")) +
  theme_bw()
...
prev_distribution_wrap   <- patchwork::wrap_elements(prev_distribution_update)
...
# left_plot      <- indSpNumber.plot / prev_distribution
#   
# right_plot    <- wrap_elements(venn_by_method[['maxN']] + venn_by_method[['pres/abs']]) / wrap_elements(indval.feat1 + indval.feat2) 
# 
# left_plot | right_plot + plot_annotation(tag_levels = "A")

#wrap_plots(indSpNumber.plot, wrap_elements(venn_by_method[['maxN']] + venn_by_method[['pres/abs']]), prev_distribution,  wrap_elements(indval.feat1 + indval.feat2), design = design) + plot_annotation(tag_levels = "A")
...
indval_bin_ter_species.df$IndSp_by_method <- ifelse(indval_bin_ter_species.df$IsIndSp.x == 0 & indval_bin_ter_species.df$IsIndSp.pred_bin == 0 & indval_bin_ter_species.df$IsIndSp.pred_ter == 1, 1,
                                                    ifelse(indval_bin_ter_species.df$IsIndSp.x == 0 & indval_bin_ter_species.df$IsIndSp.pred_bin == 1 & indval_bin_ter_species.df$IsIndSp.pred_ter == 0, 2, 
                                                           ifelse(indval_bin_ter_species.df$IsIndSp.x == 1 & indval_bin_ter_species.df$IsIndSp.pred_bin == 0 & indval_bin_ter_species.df$IsIndSp.pred_ter == 0, 3,
                                                                  ifelse(indval_bin_ter_species.df$IsIndSp.x == 1 & indval_bin_ter_species.df$IsIndSp.pred_bin == 0 & indval_bin_ter_species.df$IsIndSp.pred_ter == 1, 4, 
                                                                         ifelse(indval_bin_ter_species.df$IsIndSp.x == 1 & indval_bin_ter_species.df$IsIndSp.pred_bin == 1 & indval_bin_ter_species.df$IsIndSp.pred_ter == 0, 5, 
                                                                                ifelse(indval_bin_ter_species.df$IsIndSp.x == 1 & indval_bin_ter_species.df$IsIndSp.pred_bin == 1 & indval_bin_ter_species.df$IsIndSp.pred_ter == 1, 6, NA)
                                                                         )
                                                                  )
                                                           )
                                                    )
