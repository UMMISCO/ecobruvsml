# =============================================================================
# Script name: FigS1_code_with_twinspan.R
# Author: Estephe Kana, Eugeni Belda, Edi Prifti
# Date created: 2025-02-01
# Purpose: Generate Figure S1 with Twinspan method included - Indicator species
#          overview comparing IndVal, Twinspan, bininter, and terinter methods
#          with Venn diagrams, barplots, and AUC violin plots.
# Inputs:  analysis_outputs/Indval_output_data/Indval_all_analyses_overall_data_prev_0.Rda
#          analysis_outputs/twinspan_output_data/Twinspan_all_analyses_overall_data_prev_0.Rda
#          analysis_outputs/bininter_output_data/bininter_Predomics_all_analyses_overall_data_prev_0.Rda
#          analysis_outputs/terinter_output_data/terinter_Predomics_all_analyses_overall_data_prev_0.Rda
#          analysis_outputs/bininter_output_data/ (*_prev_*.Rda files)
#          analysis_outputs/terinter_output_data/ (*_prev_*.Rda files)
# Outputs: Figures/FigureS1/FigureS1_Best_with_twinspan.pdf
# =============================================================================

# -----------------------------------------------------------------------------
# Commands to run the script:
# cd /home/estephe/Documents/Papers/2026-aime_key_fish_bruvs/Bruvs-analysis-code/analyses/Figures/FigureS1
# Rscript FigS1_code_with_twinspan.R
# -----------------------------------------------------------------------------
...
path <- file.path(base_dir, "analyses", "Figures", "FigureS1")
...
# load indval results
x <- load(file=file.path(base_dir, "analyses", "analysis_outputs", "Indval_output_data", "Indval_all_analyses_overall_data_prev_0.Rda"))
for(i in x)
{
...
x <- load(file=file.path(base_dir, "analyses", "analysis_outputs", "twinspan_output_data", "Twinspan_all_analyses_overall_data_prev_0.Rda"))
for(i in x)
{
...
x <- load(file=file.path(base_dir, "analyses", "analysis_outputs", "bininter_output_data", "bininter_Predomics_all_analyses_overall_data_prev_0.Rda"))
for(i in x)
{
...
x <- load(file=file.path(base_dir, "analyses", "analysis_outputs", "terinter_output_data", "terinter_Predomics_all_analyses_overall_data_prev_0.Rda"))
for(i in x)
{
...
x <- list.files(path = file.path(base_dir, "analyses", "analysis_outputs", "bininter_output_data"), pattern = "*_prev_*", full.names = TRUE)
bininterObj <- list()
for(i in x)
{
  print(i)
...
x <- list.files(path = file.path(base_dir, "analyses", "analysis_outputs", "terinter_output_data"), pattern = "*_prev_*", full.names = TRUE)
x <- x[-1]
...
pdf(file=paste(path,"FigureS1_Best_with_twinspan.pdf",sep="/"), h=15, w=12)
grid.arrange(plots.list$INSHORE_OFFSHORE$barplot + ggtitle(expression(bold("A"))),
             plots.list$INSHORE_OFFSHORE$vennDiagrams_byMethod$maxN + ggtitle(expression(bold("B"))),
             plots.list$INSHORE_OFFSHORE$vennDiagrams_byMethod$`pres/abs` + ggtitle(expression(bold("C"))),
             plots.list$INSHORE_OFFSHORE$vennDiagrams_byData$bininter + ggtitle(expression(bold("D"))), 
             plots.list$INSHORE_OFFSHORE$vennDiagrams_byData$indval + ggtitle(expression(bold("E"))),
             plots.list$INSHORE_OFFSHORE$vennDiagrams_byData$terinter + ggtitle(expression(bold("F"))),
             plots.list$INSHORE_OFFSHORE$vennDiagrams_byData$twinspan + ggtitle(expression(bold("G"))),
             vplots$INSHORE_OFFSHORE + ggtitle(expression(bold("H"))), 
             layout_matrix=hlay)
dev.off()
