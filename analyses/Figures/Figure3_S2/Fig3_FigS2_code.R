# =============================================================================
# Script name: Fig3_FigS2_code.R
# Author: Estephe Kana, Eugeni Belda, Edi Prifti
# Date created: 2025-02-21
# Purpose: Generate Figure 3 - Indicator species comparison across methods
#          (IndVal, bininter, terinter), including Venn diagrams, barplots,
#          prevalence plots, correlation plots, and PERMANOVA results.
...
# Outputs: Figures/Figure3_S2/Figure3.pdf
#          Figures/Figure3_S2/Figure3_A.pdf
#          Figures/Figure3_S2/Figure3_Best.pdf
#          Figures/Figure3_S2/FigureS2.pdf
# =============================================================================

# -----------------------------------------------------------------------------
# Commands to run the script:
# cd /home/estephe/Documents/Papers/2026-aime_key_fish_bruvs/Bruvs-analysis-code/analyses/Figures/Figure3_S2
# Rscript Fig3_FigS2_code.R
# -----------------------------------------------------------------------------
...
for(i in unique(mergedf.all$comparison))
{
  ...
  plots.list[[i]][["vennDiagrams_byMethod"]] <- lapply(i.listVenn, function(x){ggvenn(x,fill_color = c("#648C67","#8195B2","#CC3837"), stroke_size = 0.5, set_name_size = 3.5,show_percentage = FALSE, stroke_alpha = 1) + 
  ...
  plots.list[[i]][["vennDiagrams_byData"]] <- lapply(i.listVenn2, function(x){ggvenn(x, stroke_size = 0.5, set_name_size = 3.5,show_percentage = FALSE, stroke_alpha = 1) + 
      theme(plot.title = element_text(hjust = 0.5),
            # legend.title = element_text(color = "black", size = 9, margin = margin(0, 10, 0, 0), hjust = 0.5),
            legend.position = "none")  + 
      coord_cartesian(clip = "off") +
...
for(i in unique(mergedf.all$comparison))
{
  ...
  boxplots.featImp[[i]][["bininter"]][["mda"]] <- ggplot(idf1.dcast, aes(x=class, y=bininter.FI)) + 
    ...
    stat_compare_means(aes(group=class), 
                       method = "wilcox", 
                       comparisons = combn(x = unique(idf1.dcast$class), m = 2, simplify = FALSE),
  ...
  boxplots.featImp[[i]][["terinter"]][["indval"]] <- ggplot(idf1.dcast, aes(x=class, y=indval.FI)) + 
    ...
    stat_compare_means(aes(group=class), 
                       method = "wilcox.test", 
                       comparisons = combn(x = unique(idf1.dcast$class), m = 2, simplify = FALSE),
                       label = "p.signif",hide.ns = FALSE, exact=FALSE) + 
    theme_bw() + 
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
          legend.position = "none")
}
...
hlay <- rbind(c(1,1,2,2,3,3),
              c(4,4,5,5,6,7))
pdf(file=file.path(base_dir, "analyses", "Figures", "Figure3_S2", "Figure3.pdf"), h=8, w=12)
...
hlay <- rbind(c(1,1,2,3,4,4),
              ...
              c(5,5,6,6,7,8),
              c(5,5,6,6,7,8))
pdf(file=file.path(base_dir, "analyses", "Figures", "Figure3_S2", "Figure3_A.pdf"), h=8, w=12)
grid.arrange(plots.list$INSHORE_OFFSHORE$barplot + ggtitle("A"),
             plots.list$INSHORE_OFFSHORE$vennDiagrams_byMethod$maxN + ggtitle("B"),
             plots.list$INSHORE_OFFSHORE$vennDiagrams_byMethod$`pres/abs` + ggtitle("C"),
             adonis.plots$INSHORE_OFFSHORE + ggtitle("D"),
...
hlay <- rbind(c(1,1,2,3,4,8,8),
              c(1,1,5,6,7,8,8),
              c(9,9,10,10,10,11,12),
              c(9,9,10,10,10,11,12))
pdf(file=file.path(base_dir, "analyses", "Figures", "Figure3_S2", "Figure3_Best.pdf"), h=9, w=14)
grid.arrange(plots.list$INSHORE_OFFSHORE$barplot + ggtitle("A"),
             plots.list$INSHORE_OFFSHORE$vennDiagrams_byMethod$maxN + ggtitle("B"),
             nullGrob(),
             plots.list$INSHORE_OFFSHORE$vennDiagrams_byMethod$`pres/abs` + ggtitle("C"),
...
hlay <- 
 ...
 AAEFGHH
 IIJJJKL"
pdf(file=file.path(base_dir, "analyses", "Figures", "Figure3_S2", "Figure3_Best.pdf"), h=8, w=12)
...
pdf(file=file.path(base_dir, "analyses", "Figures", "Figure3_S2", "FigureS2.pdf"), h=15, w=10)
hlay <- rbind(c(1,1,1),
              c(2,2,NA),
              c(3,4,5),
              c(3,4,5))
