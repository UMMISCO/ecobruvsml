# =============================================================================
# Script name: Fig3_FigS2_code_with_twinspan.R
# Author: Estephe Kana, Eugeni Belda, Edi Prifti
# Date created: 2025-02-01
# Purpose: Generate Figure 3 and Figure S2 with Twinspan method included -
#          Indicator species comparison across methods (IndVal, Twinspan,
#          bininter, terinter), including Venn diagrams, barplots, prevalence
#          plots, correlation plots, and PERMANOVA results.
...
# Outputs: Figures/Figure3_S2/Figure3_Best_with_twinspan.pdf
#          Figures/Figure3_S2/FigureS2_Best_with_twinspan.pdf
# =============================================================================

# -----------------------------------------------------------------------------
# Commands to run the script:
# cd /home/estephe/Documents/Papers/2026-aime_key_fish_bruvs/Bruvs-analysis-code/analyses/Figures/Figure3_S2
# Rscript Fig3_FigS2_code_with_twinspan.R
# -----------------------------------------------------------------------------
...
dir.create(file.path(base_dir, "analyses", "Figures", "Figure3_S2"), showWarnings = FALSE, recursive = TRUE)

objlist <- list()
# load indval results
x <- load(file=file.path(base_dir, "analyses", "analysis_outputs", "Indval_output_data", "Indval_all_analyses_overall_data_prev_10.Rda"))
for(i in x)
{
...
x <- load(file=file.path(base_dir, "analyses", "analysis_outputs", "twinspan_output_data", "Twinspan_all_analyses_overall_data_prev_10.Rda"))
for(i in x)
{
  objlist[["Twinspan"]][[i]] <- get(i)
}
rm(list = x)

# load bininter results
x <- load(file=file.path(base_dir, "analyses", "analysis_outputs", "bininter_output_data", "bininter_Predomics_all_analyses_overall_data_prev_10.Rda"))
for(i in x)
{
...
x <- load(file=file.path(base_dir, "analyses", "analysis_outputs", "terinter_output_data", "terinter_Predomics_all_analyses_overall_data_prev_10.Rda"))
for(i in x)
{
...
mergedf.maxN <- rbind(objlist$indval$indvalout.maxn.sub[,-match("pval", colnames(objlist$indval$indvalout.maxn.sub))],
                      objlist$Twinspan$twinspan.maxn.sub[,-match("pval", colnames(objlist$Twinspan$twinspan.maxn.sub))],
...
mergedf.bin <- rbind(objlist$indval$indvalout.bin.sub[,-match("pval", colnames(objlist$indval$indvalout.bin.sub))],
                     objlist$Twinspan$twinspan.bin.sub[,-match("pval", colnames(objlist$Twinspan$twinspan.bin.sub))],
...
colours.method <- c("#648C67","#8195B2","#CC3837", "#FF7F00")
...
for(i in unique(mergedf.all$comparison))
{
  ...
  for(j in unique(idf$data))
  {
    i.listVenn[[j]] <- list(bininter=idf$feature[idf$source %in% "bininter" & idf$data %in% j],
                            ...
                            twinspan=idf$feature[idf$source %in% "twinspan" & idf$data %in% j])
  }
  plots.list[[i]][["vennDiagrams_byMethod"]] <- lapply(i.listVenn, function(x){ggvenn(x,fill_color = c("#648C67","#8195B2","#CC3837", "#FF7F00"), show_stats = "c", stroke_size = 0.5, set_name_size = 4.5, stroke_alpha = 1, padding = 0.2) + 
  ...
  plots.list[[i]][["vennDiagrams_byData"]] <- purrr::imap(i.listVenn2, function(x, name){
    ggvenn(x, stroke_size = 0.5, set_name_size = 4.5, show_stats = "c", stroke_alpha = 1, padding = 0.2) + 
      labs(caption = name) +
      theme(plot.title = element_text(hjust = 0.5, vjust=1),
            plot.caption = element_text(hjust = 0.5, vjust = 1, size = 12, face = "bold"),
            legend.position = "none") +
...
adonis.bin.twinspan <- do.call("rbind", objlist$Twinspan$adonis.bin)
adonis.bin.twinspan$source <- gsub("allSpecies","allSpecies_twinspan", adonis.bin.twinspan$source)
adonis.bin <- rbind(adonis.bin.indval, adonis.bin.bininter, adonis.bin.terinter, adonis.bin.twinspan)
...
adonis.maxn.twinspan <- do.call("rbind", objlist$Twinspan$adonis.maxn)
adonis.maxn.twinspan$source <- gsub("allSpecies","allSpecies_twinspan", adonis.maxn.twinspan$source)
adonis.maxn <- rbind(adonis.maxn.indval, adonis.maxn.bininter, adonis.maxn.terinter, adonis.maxn.twinspan)

adonis.alldf <- rbind(adonis.bin, adonis.maxn)
adonis.alldf$pval.cat <- ifelse(adonis.alldf$Pr..F.<0.05, "*","")
adonis.alldf <- adonis.alldf[!adonis.alldf$source %in% c("allSpecies_bininter","allSpecies_terinter", "allSpecies_twinspan"),]
adonis.alldf$source <- gsub("Species"," Species", adonis.alldf$source)
adonis.alldf$source <- factor(adonis.alldf$source, levels = c("all Species","terinter Species", "indval Species", "bininter Species", "twinspan Species"))
...
for(i in unique(mergedf.all$comparison)) {
  ...
  ilist.bininter.indval <- do.call("rbind", ilist.bininter)
  ...
  boxplots.featImp[[i]][["bininter"]][["mda.indval"]] <- ggplot(idf1.dcast, aes(x=class, y=bininter.FI)) + 
  ...
  for(facet_name in names(facet_comparisons)) {
    if(!is.null(facet_comparisons[[facet_name]])) {
      boxplots.featImp[[i]][["bininter"]][["mda.indval"]] <- boxplots.featImp[[i]][["bininter"]][["mda.indval"]] +
        stat_compare_means(
  ...
  ##twinspan-bininter
  idf1 <- idf[idf$source %in% c("twinspan","bininter"),]
  idf1.dcast <- dcast(data = idf1, formula = feature+data~source, value.var="featureImportance")
  idf1.dcast2 <- dcast(data = idf1, formula = feature+data~source, value.var="IsIndSp")
  idf1.dcast2$class <- ifelse((idf1.dcast2$bininter==1 & idf1.dcast2$twinspan==1),"common",
                              ifelse((idf1.dcast2$bininter==1 & idf1.dcast2$twinspan==0),"bininterOnly",
                                     ifelse(idf1.dcast2$bininter==0 & idf1.dcast2$twinspan==1,"twinspanOnly",NA)))
  idf1.dcast2 <- idf1.dcast2[!is.na(idf1.dcast2$class),]
  ...
  for(j in unique(idf1.dcast2$class))
  {
    ...
    for(m in unique(idf1.dcast2$data))
    {
      ...
      {
        jfeats.corr <- cor.test(idf1.dcast$bininter[idf1.dcast$feature %in% jfeats & idf1.dcast$data %in% m], 
                                idf1.dcast$twinspan[idf1.dcast$feature %in% jfeats & idf1.dcast$data %in% m], 
                                method="spearman")
        jfeats.corr.df <- data.frame(comparison="twinspan-bininter",
                                     feature.class=j,
                                     dataSource=m,
                                     rho=jfeats.corr$estimate,
                                     pval=jfeats.corr$p.value,
  ...
  ilist.bininter.twinspan <- do.call("rbind", ilist.bininter)
  ...
  # Create the plot with facet-specific comparisons
  boxplots.featImp[[i]][["bininter"]][["mda.twinspan"]] <- ggplot(idf1.dcast, aes(x=class, y=bininter.FI)) + 
    geom_boxplot(outlier.shape = NA) + 
    geom_point() + 
    facet_grid(.~data, scales = "free_x", space = "free_x") + 
    theme_bw() + 
  ...
  for(facet_name in names(facet_comparisons)) {
    if(!is.null(facet_comparisons[[facet_name]])) {
      boxplots.featImp[[i]][["bininter"]][["mda.twinspan"]] <- boxplots.featImp[[i]][["bininter"]][["mda.twinspan"]] +
        stat_compare_means(
          data = idf1.dcast[idf1.dcast$data == facet_name,],
          aes(group = class),
          method = "wilcox.test",
  ...
  # Repeat for twinspan.FI plot
  boxplots.featImp[[i]][["bininter"]][["twinspan"]] <- ggplot(idf1.dcast, aes(x=class, y=twinspan.FI)) + 
    geom_boxplot(outlier.shape = NA) + 
    geom_point() + 
    facet_grid(.~data, scales = "free_x", space = "free_x") + 
    theme_bw() + 
  ...
  for(facet_name in names(facet_comparisons)) {
    if(!is.null(facet_comparisons[[facet_name]])) {
      boxplots.featImp[[i]][["bininter"]][["twinspan"]] <- boxplots.featImp[[i]][["bininter"]][["twinspan"]] +
        stat_compare_means(
          data = idf1.dcast[idf1.dcast$data == facet_name,],
          aes(group = class),
          method = "wilcox.test",
  ...
  ilist.terinter.indval <- do.call("rbind", ilist.terinter)
  ##boxplots featureImportance by feature class
  idf1.dcast$mvar <- paste(idf1.dcast$feature, idf1.dcast$data)
  colnames(idf1.dcast)[3:4] <- paste(colnames(idf1.dcast)[3:4],".FI", sep = "")
  idf1.dcast2$mvar <- paste(idf1.dcast2$feature, idf1.dcast2$data)
  ...
  boxplots.featImp[[i]][["terinter"]][["mda.indval"]] <- ggplot(idf1.dcast, aes(x=class, y=terinter.FI)) + 
    geom_boxplot(outlier.shape = NA) + 
    geom_point() + 
    facet_grid(.~data, scales = "free_x", space = "free_x") + 
    theme_bw() + 
  ...
  for(facet_name in names(facet_comparisons)) {
    if(!is.null(facet_comparisons[[facet_name]])) {
      boxplots.featImp[[i]][["terinter"]][["mda.indval"]] <- boxplots.featImp[[i]][["terinter"]][["mda.indval"]] +
        stat_compare_means(
          data = idf1.dcast[idf1.dcast$data == facet_name,],
          aes(group = class),
          method = "wilcox.test",
  ...
  ##twinspan-terinter (similar modifications as above)
  idf1 <- idf[idf$source %in% c("twinspan","terinter"),]
  idf1.dcast <- dcast(data = idf1, formula = feature+data~source, value.var="featureImportance")
  idf1.dcast2 <- dcast(data = idf1, formula = feature+data~source, value.var="IsIndSp")
  idf1.dcast2$class <- ifelse((idf1.dcast2$terinter==1 & idf1.dcast2$twinspan==1),"common",
                              ifelse((idf1.dcast2$terinter==1 & idf1.dcast2$twinspan==0),"terinterOnly",
                                     ifelse(idf1.dcast2$terinter==0 & idf1.dcast2$twinspan==1,"twinspanOnly",NA)))
  idf1.dcast2 <- idf1.dcast2[!is.na(idf1.dcast2$class),]
  ilist.terinter <- list()
  for(j in unique(idf1.dcast2$class))
  {
    ...
    for(m in unique(idf1.dcast2$data))
    {
      ...
      if(length(jfeats)>1)
      {
        jfeats.corr <- cor.test(idf1.dcast$terinter[idf1.dcast$feature %in% jfeats & idf1.dcast$data %in% m],
                                idf1.dcast$twinspan[idf1.dcast$feature %in% jfeats & idf1.dcast$data %in% m],
                                method="spearman")
        jfeats.corr.df <- data.frame(comparison="twinspan-terinter",
                                     feature.class=j,
                                     dataSource=m,
                                     rho=jfeats.corr$estimate,
                                     pval=jfeats.corr$p.value,
  ...
  ilist.terinter.twinspan <- do.call("rbind", ilist.terinter)
  ##boxplots featureImportance by feature class
  idf1.dcast$mvar <- paste(idf1.dcast$feature, idf1.dcast$data)
  colnames(idf1.dcast)[3:4] <- paste(colnames(idf1.dcast)[3:4],".FI", sep = "")
  idf1.dcast2$mvar <- paste(idf1.dcast2$feature, idf1.dcast2$data)
  ...
  # Create terinter plots with facet-specific comparisons
  boxplots.featImp[[i]][["terinter"]][["mda.twinspan"]] <- ggplot(idf1.dcast, aes(x=class, y=terinter.FI)) + 
    geom_boxplot(outlier.shape = NA) + 
    geom_point() + 
    facet_grid(.~data, scales = "free_x", space = "free_x") + 
    theme_bw() + 
  ...
  for(facet_name in names(facet_comparisons)) {
    if(!is.null(facet_comparisons[[facet_name]])) {
      boxplots.featImp[[i]][["terinter"]][["mda.twinspan"]] <- boxplots.featImp[[i]][["terinter"]][["mda.twinspan"]] +
        stat_compare_means(
          data = idf1.dcast[idf1.dcast$data == facet_name,],
          aes(group = class),
          method = "wilcox.test",
  ...
  boxplots.featImp[[i]][["terinter"]][["twinspan"]] <- ggplot(idf1.dcast, aes(x=class, y=twinspan.FI)) + 
    geom_boxplot(outlier.shape = NA) + 
    geom_point() + 
    facet_grid(.~data, scales = "free_x", space = "free_x") + 
    theme_bw() + 
  ...
  for(facet_name in names(facet_comparisons)) {
    if(!is.null(facet_comparisons[[facet_name]])) {
      boxplots.featImp[[i]][["terinter"]][["twinspan"]] <- boxplots.featImp[[i]][["terinter"]][["twinspan"]] +
        stat_compare_means(
          data = idf1.dcast[idf1.dcast$data == facet_name,],
          aes(group = class),
          method = "wilcox.test",
  ...
  corr.list[[i]] <- rbind(ilist.terinter.indval,
                          ilist.terinter.twinspan,
                          ilist.bininter.indval,
                          ilist.bininter.twinspan)
}
...
grid.arrange(boxplots.featImp$INSHORE_OFFSHORE$bininter$mda.indval,
             boxplots.featImp$INSHORE_OFFSHORE$bininter$indval,
             boxplots.featImp$INSHORE_OFFSHORE$terinter$mda.indval,
             boxplots.featImp$INSHORE_OFFSHORE$terinter$indval, ncol=2, nrow=2)

hlay <- rbind(c(1,1,1,1,1,2,2,2,2,4,4,6,6,8,8,8,8,8),
              c(1,1,1,1,1,3,3,3,3,5,5,7,7,8,8,8,8,8),
              c(9,9,9,9,10,10,10,10,10,10,11,11,12,12,13,13,14,14),
              c(9,9,9,9,10,10,10,10,10,10,11,11,12,12,13,13,14,14))
pdf(file=file.path(base_dir, "analyses", "Figures", "Figure3_S2", "Figure3_Best_with_twinspan.pdf"), h=15, w=20)
grid.arrange(plots.list$INSHORE_OFFSHORE$barplot + ggtitle("A"),
             ...
             plots.list$INSHORE_OFFSHORE$vennDiagrams_byData$twinspan + ggtitle("G"),
             adonis.plots$INSHORE_OFFSHORE + ggtitle("H"),
             prevplots$INSHORE_OFFSHORE + ggtitle("I"),
             corr.plots$INSHORE_OFFSHORE + ggtitle("J"), 
             boxplots.featImp$INSHORE_OFFSHORE$bininter$mda.indval + ggtitle("K"),
             boxplots.featImp$INSHORE_OFFSHORE$terinter$mda.indval + ggtitle("L"),
             boxplots.featImp$INSHORE_OFFSHORE$bininter$mda.twinspan + ggtitle("M"),
             boxplots.featImp$INSHORE_OFFSHORE$terinter$mda.twinspan + ggtitle("N"), layout_matrix=hlay)
dev.off()
...
boxplots.featImp.allBin.mda <- list()
boxplots.featImp.allBin.mda.plot <- list()

boxplots.featImp.allBin.mda[["indval"]] <- Map(function(x, nm) {
  y <- x[["bininter"]][["mda.indval"]][["data"]]
  y[["comparison"]] <- nm
  y
}, boxplots.featImp, names(boxplots.featImp))

boxplots.featImp.allBin.mda[["twinspan"]] <- Map(function(x, nm) {
  y <- x[["bininter"]][["mda.twinspan"]][["data"]]
  y[["comparison"]] <- nm
  y
}, boxplots.featImp, names(boxplots.featImp))
...
{
  boxplots.featImp.allBin.mda[[i]] <- do.call("rbind", boxplots.featImp.allBin.mda[[i]])
  # boxplots.featImp.allBin.mda[[i]][,"comparison"] <- i
  
  # Extract 4th column name dynamically
  y_var <- names(boxplots.featImp.allBin.mda[[i]])[4]
  
  plot_data <- boxplots.featImp.allBin.mda[[i]][
    !boxplots.featImp.allBin.mda[[i]]$comparison %in% "INSHORE_OFFSHORE", 
  ]
  
  boxplots.featImp.allBin.mda.plot[[i]] <- ggplot(plot_data, aes(x = class, y = .data[[y_var]])) + 
    geom_boxplot(outlier.shape = NA) + 
    geom_point() + 
    facet_grid(data ~ comparison, scales = "free_x", space = "free_x") + 
    stat_compare_means(
      aes(group = class), 
      method = "wilcox", 
      comparisons = combn(x = unique(plot_data$class), m = 2, simplify = FALSE),
      label = "p.signif", 
      vjust = 1.5
    ) + 
    theme_bw() + 
...
boxplots.featImp.allTer.mda <- list()
boxplots.featImp.allTer.mda.plot <- list()

boxplots.featImp.allTer.mda[["indval"]] <- Map(function(x, nm) {
  y <- x[["terinter"]][["mda.indval"]][["data"]]
  y[["comparison"]] <- nm
  y
}, boxplots.featImp, names(boxplots.featImp))

boxplots.featImp.allTer.mda[["twinspan"]] <- Map(function(x, nm) {
  y <- x[["terinter"]][["mda.twinspan"]][["data"]]
  y[["comparison"]] <- nm
  y
}, boxplots.featImp, names(boxplots.featImp))
...
{
  boxplots.featImp.allTer.mda[[i]] <- do.call("rbind", boxplots.featImp.allTer.mda[[i]])
  # boxplots.featImp.allBin.mda[[i]][,"comparison"] <- i
  
  # Extract 4th column name dynamically
  y_var <- names(boxplots.featImp.allTer.mda[[i]])[4]
  
  plot_data <- boxplots.featImp.allTer.mda[[i]][
    !boxplots.featImp.allTer.mda[[i]]$comparison %in% "INSHORE_OFFSHORE", 
  ]
  
  boxplots.featImp.allTer.mda.plot[[i]] <- ggplot(plot_data, aes(x = class, y = .data[[y_var]])) + 
    geom_boxplot(outlier.shape = NA) + 
    geom_point() + 
    facet_grid(data ~ comparison, scales = "free_x", space = "free_x") + 
    stat_compare_means(
...
pdf(file=file.path(base_dir, "analyses", "Figures", "Figure3_S2", "FigureS2_Best_with_twinspan.pdf"), h=14, w=14)
hlay <- rbind(c(1,1,1,1,2,2,2,2),
              c(1,1,1,1,2,2,2,2),
              c(4,4,4,4,3,3,3,3),
              c(4,4,4,4,3,3,3,3),
              c(4,4,4,4,3,3,3,3),
...
grid.arrange(plots.list.barplot.all.plot + ggtitle("A"),
             adonisplot + ggtitle("B") + theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "right"),
             corr.plots.all.plot + ggtitle("C"),
             prevplots.all.plot + ggtitle("D"),
             boxplots.featImp.allBin.mda.plot$indval + ylab("MDA; Bininter - Indval") + ggtitle("E"),
             boxplots.featImp.allTer.mda.plot$indval + ylab("MDA; Terinter - Indval") + ggtitle("F"),
             boxplots.featImp.allBin.mda.plot$twinspan + ylab("MDA; Bininter - Twinspan") + ggtitle("G"),
             boxplots.featImp.allTer.mda.plot$twinspan + ylab("MDA; Terinter - Twinspan") + ggtitle("H"),
             layout_matrix=hlay)
dev.off()
