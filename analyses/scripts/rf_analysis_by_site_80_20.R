# =============================================================================
# Script name: rf_analysis_by_site_80_20.R
# Author: Estephe Kana, Eugeni Belda, Edi Prifti
# Date created: 2024-09-01
# Purpose: Evaluate Random Forest classifiers using IndVal indicator species as features, on 80/20 train/test split data.
# Inputs:  analyses/analysis_outputs/Indval_output_data/analysis_80_20/ (loaded .Rda files)
# Outputs: analyses/analysis_outputs/rf_analysis_results_80_20.Rda
# =============================================================================

# -----------------------------------------------------------------------------
# Commands to run the script:
# cd /home/estephe/Documents/Papers/2026-aime_key_fish_bruvs/Bruvs-analysis-code/analyses/scripts
# Rscript rf_analysis_by_site_80_20.R
# -----------------------------------------------------------------------------
...
# Extract all indicator species from alldatalist.video_80_20 and save as CSV
all_species.rows <- list()
for (seed in names(alldatalist.video_80_20)) {
  for (prev_rate in names(alldatalist.video_80_20[[seed]])) {
    for (method in c("Indval", "bininter", "terinter")) {
      data_m <- alldatalist.video_80_20[[seed]][[prev_rate]][[method]]
      if (is.null(data_m)) next
      sp_field <- if (method == "Indval") "indval_pval" else "pred_out_fbm"
      for (src in c("maxN", "presAbs")) {
        sub_list <- if (method == "Indval") {
          if (src == "maxN") data_m$indvalout.maxn else data_m$indvalout.bin
        } else {
          if (src == "maxN") data_m$predout.maxn else data_m$predout.bin
        }
        for (comp in names(sub_list)) {
          sp_df <- sub_list[[comp]][[sp_field]]
          if (is.null(sp_df) || nrow(sp_df) == 0) next
          sp_df$seed       <- seed
          sp_df$prev_rate  <- prev_rate
          sp_df$source     <- tolower(method)
          sp_df$data       <- src
          sp_df$comparison <- comp
          all_species.rows[[length(all_species.rows) + 1]] <- sp_df
        }
      }
    }
  }
}
all_species_80_20.df <- do.call(rbind, all_species.rows)
rownames(all_species_80_20.df) <- NULL
indics_species_80_20.df <- all_species_80_20.df[all_species_80_20.df$IsIndSp == 1, ]
write.csv(indics_species_80_20.df,
          file = file.path(root_path, "indics_species_analysis_results_80_20.csv"),
          row.names = FALSE)
...
# save rf results
save(rf.metrics.df, file = paste(root_path, "rf_analysis_results_80_20.Rda", sep="/"))
