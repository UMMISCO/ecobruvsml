# =============================================================================
# Script name: get_plots_results_80_20.R
# Author: Estephe Kana, Eugeni Belda, Edi Prifti
# Date created: 2024-12-01
# Purpose: Generate publication figures (PERMANOVA, Venn diagrams, feature importance) for 80/20 train/test split results across all methods including TWINSPAN.
# Inputs:  analyses/analysis_outputs/all_analysis_results_with_twinspan_80_20.Rda
# Outputs: PDF plots in analyses/Figures/, CSV files in analyses/analysis_outputs/
# =============================================================================

# -----------------------------------------------------------------------------
# Commands to run the script:
# cd /home/estephe/Documents/Papers/2026-aime_key_fish_bruvs/Bruvs-analysis-code/analyses/scripts
# Rscript get_plots_results_80_20.R
# -----------------------------------------------------------------------------
...
all_species_IN_OFF.df[all_species_IN_OFF.df == 'twinspan'] <- 'twinspan'
...
all_indics_species_IN_OFF.df[all_indics_species_IN_OFF.df == 'twinspan'] <- 'twinspan'
...
venn_source_plots <- lapply(names(venn_by_source), function(x) venn_by_source[[x]])
venn_by_source <- wrap_plots(venn_source_plots, ncol = length(venn_source_plots))
...
all_species_IN_OFF_twinspan.df= all_species_IN_OFF.df[all_species_IN_OFF.df$source=="twinspan",]
## indval vs bininter
...
twinspan_species = all_species_IN_OFF_twinspan.df[all_species_IN_OFF_twinspan.df$IsIndSp==1,]
...
if (nrow(twinspan_species) > 0) {
  twinspan_data.maxn = filter(twinspan_species, data == "maxN")
  twinspan_data.bin = filter(twinspan_species, data == "pres/abs")
  twinspan_data.maxn.X = community.maxn.X[, colnames(community.maxn.X) %in% twinspan_data.maxn$feature]
  twinspan_data.bin.X = community.bin.X[, colnames(community.bin.X) %in% twinspan_data.bin$feature]
  twinspan_data.maxn.X = twinspan_data.maxn.X[rowSums(twinspan_data.maxn.X > 0) > 0, , drop = FALSE]
  twinspan_data.bin.X = twinspan_data.bin.X[rowSums(twinspan_data.bin.X > 0) > 0, , drop = FALSE]
  if (ncol(twinspan_data.maxn.X) > 1 && nrow(twinspan_data.maxn.X) > 1) {
    nmds_data[["maxN"]][["twinspan"]] = metaMDS(twinspan_data.maxn.X, distance = "bray")
  }
  if (ncol(twinspan_data.bin.X) > 1 && nrow(twinspan_data.bin.X) > 1) {
    nmds_data[["presAbs"]][["twinspan"]] = metaMDS(twinspan_data.bin.X, distance = "jaccard")
  }
}
...
nmds_abund_items <- unname(nmds.plots$maxN)
nmds_presabs_items <- unname(nmds.plots$presAbs)
nmds_abund <- do.call(grid.arrange, c(nmds_abund_items, ncol = 3))
nmds_presAbs <- do.call(grid.arrange, c(nmds_presabs_items, ncol = 3))
