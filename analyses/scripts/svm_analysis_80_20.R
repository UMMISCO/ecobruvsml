# =============================================================================
# Script name: svm_analysis_80_20.R
# Author: Estephe Kana, Eugeni Belda, Edi Prifti
# Date created: 2024-09-01
# Purpose: Evaluate Support Vector Machine (SVM) classifiers using IndVal indicator species as features, on 80/20 train/test split data.
# Inputs:  analyses/analysis_outputs/Indval_output_data/analysis_80_20/ (loaded .Rda files)
# Outputs: analyses/analysis_outputs/svm_analysis_results_80_20.Rda
# =============================================================================

# -----------------------------------------------------------------------------
# Commands to run the script:
# cd /home/estephe/Documents/Papers/2026-aime_key_fish_bruvs/Bruvs-analysis-code/analyses/scripts
# Rscript svm_analysis_80_20.R
# -----------------------------------------------------------------------------
...
svm_twinspan_analysis <- function(data, source, comp) {
  ...
  twinspan_test.auc <- performance(twinspan_svm.test.prediction, measure = "auc")@y.values[[1]]
...
for (seed in names(alldatalist.video_80_20)) {
  for (prev_rate in c("prev_0", "prev_10")) {
    for (model in c("Indval","bininter","terinter","Twinspan")) {
      for (comp in c("BAR_BAY", "BAR_LAG", "BAY_LAG", "INSHORE_OFFSHORE")){
        ...
        } else if (model == "Twinspan") {
          results_80_20 = alldatalist.video_80_20[[seed]][[prev_rate]][[model]]
          svm.results = svm_twinspan_analysis(results_80_20, source = "maxN", comp)
          svm.results$prev_rate = prev_rate
          svm.results$seed = seed
          svm.results$method = paste("SVM", model, sep = "_")
          svm.metrics.list[[comp]][["maxN"]][[model]][[prev_rate]][[seed]] <- svm.results

          svm.results = svm_twinspan_analysis(results_80_20, source = "presAbs", comp)
          svm.results$prev_rate = prev_rate
          svm.results$seed = seed
          svm.results$method = paste("SVM", model, sep = "_")
          svm.metrics.list[[comp]][["presAbs"]][[model]][[prev_rate]][[seed]] <- svm.results
